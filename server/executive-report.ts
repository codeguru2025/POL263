...
export async function buildExecutiveReport(orgId: string, params: ExecutiveReportParams) {
  ...
  const productRevenueRows = await tdb.execute(sql`
  ...
  const funeralByType = !hasFuneralOps ? [] : await tdb
  ...
  const funeralByBranchRows = !hasFuneralOps ? [] : await tdb.execute(sql`
  ...
  const funeralByLocationRows = !hasFuneralOps ? [] : await tdb.execute(sql`
  ...
  const crossBorderSplitRows = !hasFuneralOps ? [] : await tdb.execute(sql`
  ...
  const funeralTrendRows = !hasFuneralOps ? [] : await tdb.execute(sql`
  ...
  // converted" metric) ──
  const quoteStatsRows = !hasFuneralOps ? [] : await tdb.execute(sql`
  ...
  const mortuaryByScope = !hasFuneralOps ? [] : await tdb
  ...
  const revenueByStreamRows = !hasFuneralOps ? [] : await tdb.execute(sql`
  ...
  const storageFeeRows = !hasFuneralOps ? [] : await tdb.execute(sql`
  ...
  `);
  const chapelFeeRows = !hasFuneralOps ? [] : await tdb.execute(sql`
  ...
  const fuelCostRows = !hasFleet ? [] : await tdb.execute(sql`
    ...
    GROUP BY currency
  `);
  const maintenanceCostRows = !hasFleet ? [] : await tdb.execute(sql`
    SELECT currency, COALESCE(SUM(cost_amount::numeric), 0) AS total
  ...
  if (hasClaims) {
    const openClaimsRows = await tdb.execute(sql`
