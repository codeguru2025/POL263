...
export async function streamPolicyDocumentToResponse(policyId: string, orgId: string, res: Response, options?: { attachment?: boolean; lang?: string }): Promise<void> {
  ...
  if (terms.length > 0) {
    ...
    for (let i = 0; i < terms.length; i++) {
      ...
      // ── Term section header ───────────────────────────────────
      doc.rect(50, y, 495, 18).fill(C_PRIMARY);
      doc.font("Helvetica-Bold").fontSize(9).fillColor("#ffffff")
        .text(`${i + 1}. ${translatedTitles[i].toUpperCase()}`, 58, y + 4, { width: 479 });
      y += 22;
      doc.fillColor(C_TEXT);

      // ── Term body — formatted ─────────────────────────────────
      // Use PDFKit's natural flow for body text so it paginates automatically.
      doc.y = y;
      doc.x = 50;
      renderFormattedTermContent(doc, translatedContents[i], 58, 479);
      y = doc.y + 10;

      // Light separator between terms
      if (i < terms.length - 1) {
        doc.moveTo(50, y).lineTo(545, y).lineWidth(0.3).strokeColor(C_BORDER).stroke();
        y += 8;
      }
    }
  }

  // ── Footer zone: signature + stamp + QR ──────────────────────
  // Always start the footer on a fresh section; if content is close to
  // the bottom of the page, push to a new page.
  const FOOTER_H = 140; // height reserved for the footer zone
  if (y + FOOTER_H > doc.page.height - 50) {
    doc.addPage();
    y = 50;
  }
  ...
  // Divider
  doc.moveTo(50, footerTop).lineTo(545, footerTop).lineWidth(0.5).strokeColor(C_BORDER).stroke();

  const panelY = footerTop + 12;

  // Signature (left)
  doc.font("Helvetica").fontSize(7.5).fillColor(C_MUTED).text("Authorised Signature", 50, panelY, { width: SIG_W });
  if (signatureBuffer) {
    try { doc.image(signatureBuffer, 50, panelY + 12, { width: SIG_W, height: SIG_H, fit: [SIG_W, SIG_H] }); } catch (_) {}
  }
  doc.moveTo(50, panelY + 12 + SIG_H + 2).lineTo(50 + SIG_W, panelY + 12 + SIG_H + 2)
    .lineWidth(0.5).strokeColor(C_BORDER).stroke();

  // Company stamp (centre)
  const stampCx = (50 + 545) / 2;
  const stampCy = panelY + STAMP_R + 12;
  drawCompanyStamp(doc, org?.name || "POL263", stampCx, stampCy, STAMP_R);

  // Verification QR (right)
  if (policyQrBuffer) {
    drawVerifyQrPanel(doc, policyQrBuffer, 545 - QR_SIZE, panelY, QR_SIZE);
  }

  // Footer text
  const fTextY = panelY + STAMP_R * 2 + 24;
  doc.moveTo(50, fTextY).lineTo(545, fTextY).lineWidth(0.3).strokeColor(C_BORDER).stroke();
  doc.font("Helvetica-Bold").fontSize(7.5).fillColor(C_PRIMARY)
    .text(org?.footerText || `${org?.name || "POL263"} — All rights reserved`, 50, fTextY + 5, { align: "center", width: 495 });
  doc.font("Helvetica").fontSize(6.5).fillColor(C_MUTED)
    .text(
      `Generated by ${org?.name || "POL263"} · ${new Date().toLocaleDateString("en-GB")} · This document is electronically verified`,
      50, fTextY + 16, { align: "center", width: 495 },
    );
