...
import { Building2, Users, UserCircle, Loader2 } from "lucide-react";
...
import MarketingHome from "@/pages/public/marketing-home";

type TenantContext = { id: string; name: string; slug: string } | { isPlatformAdminHost: true } | null;
...
export default function Home() {
  ...
  const { data: tenantCtx } = useQuery<TenantContext>({
  ...
  const isPlatformAdminHost = !!tenantCtx && "isPlatformAdminHost" in tenantCtx;
  const tenantId = (tenantCtx && "id" in tenantCtx ? tenantCtx.id : null) ?? null;
