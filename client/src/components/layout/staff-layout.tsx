    ...
    : [
        {
          title: "Sales",
          items: filterNav([
            { href: "/staff/leads", label: "Leads / Pipeline", icon: Target, permission: "read:lead" },
            { href: "/staff/policies", label: "New Policy", icon: FileStack, permission: "write:policy" },
          ]),
        },
        {
          title: "Clients",
      ...
      <header className="border-b bg-card shrink-0 z-30">
        <div className={cn(APP_SHELL_MAX, "px-3 sm:px-4 py-2.5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between")}>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-xs sm:text-sm text-muted-foreground justify-start lg:justify-end">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                {!isControlPlaneMode && (
                  <>
                    <DropdownMenuItem asChild>
                      <Link href="/staff/reminders" className="cursor-pointer">Reminders</Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild>
                      <Link href="/staff/help" className="cursor-pointer">Help Centre</Link>
                    </DropdownMenuItem>
                  </>
                )}
                <DropdownMenuItem
            ...
            {quickCreate && hasTenant && !isControlPlaneMode && <QuickCreateMenu />}
